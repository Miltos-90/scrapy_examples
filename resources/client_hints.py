...
from definitions  import UNKNOWN_NAME, UNKNOWN_VERSION
...
class ClientHintGenerator():
    ...
    def __call__(self, userAgent: str) -> Dict[str, str]:
        ...
        # Extract required dataclass properties
        # If unknown tokens -> convert to None with an adapter (easier than handling an UNKNOWN, or UNKNOWN_VERSION tokens)
        # compute device type again
        #
        ...
        devModel  = getattr(device,  'type')
        osName    = getattr(os,      'name')
        osVersion = getattr(os,      'version')

        print('browser:', bName, 'browser version:', bVersion, 'cpu:', cpuArch, 'device type:', devType, 'device model:', devModel)

        # Generate client hints dictionary
        return {
            "Sec_CH_UA"                   : self._UA(bName, bMajorVer),
            "Sec_CH_UA_Arch"              : self._UAGeneric(cpuArch),
            "Sec_CH_UA_Bitness"           : self._UABitness(cpuArch),
            "Sec_CH_UA_Full_Version_List" : self._UA(bName, bVersion, full = True),
            "Sec_CH_UA_Mobile"            : self._UAMobile(devType),
            "Sec_CH_UA_Model"             : self._UAGeneric(devModel),
            "Sec_CH_UA_Platform"          : self._UAPlatform(osName),
            "Sec_CH_UA_Platform_Version"  : self._UAGeneric(osVersion),
        }
    ...
    def _UABitness(self,
        ...
        return self._format(self.cpuBitness.get(name))
    ...
    def _UAPlatform(self,
        ...
        if name is not None and name.lower() not in ['ios', 'mac os', 'macos']: 
            name = name.title()
    ...
    def _UA(self,
        ...
        if full : brandList = [ {'brand': ' Not A;Brand', 'version': '99.0.0.0'} ]
        else    : brandList = [ {'brand': ' Not A;Brand', 'version': '99'} ]
        
        if name is not None: brandList.append({'brand': name, 'version': version})
    ...
    @staticmethod
    def _format(x: Union[str, None]) -> str:
