...
class ClientHintGenerator():
    ...
    def __init__(self):
        ...
        self.cpuBitness = readFile('./data/cpu_bitness.json') # Read CPU Bitness mapper
        self.Parser     = Parser()

        return


    def __call__(self, agent: str) -> Dict[str, str]:
        """ Generates client hints from a user agent, using the dictionaries
            resulting from the parsing operation.
        """

        # Parse user agent
        browser, cpu, device, _, os = self.Parser(agent)

        # Extract required dataclass properties
        # If unknown tokens -> convert to None with an adapter
        # Browser majorVersion is not initialised
        #
