...
class GenericParser():
    ...
    def _map(match: Union[str, None], key:str, value: Union[str, Callable, None] = None) -> str:
            ...
            print('_map:', match, key, value, out)
    ...
    def _UNKtoken(key:str) -> str:
        ...
        return  defs.UNKNOWN_VERSION if key == 'version' else defs.UNKNOWN_NAME
    ...
    def _dataClassFromDict(self, argDict: dict, filter = False):
        ...
        if filter: # Remove additional args from the argDict that do not appear in the dataclass
            filteredArgDict = {k : v for k, v in argDict.items() if k in fieldSet}
        else:
            filteredArgDict = argDict
            
        # make dataclasss
        print('_dataClassFromDict:', argDict)
    ...
    def __call__(self, userAgent: str) -> defs.Dataclass:
        """ Searches the regex list (see regexes.py) for a match with the user
            ...
            string by the substrings returned by a regex match.
        """
