...
class SoftwareGenerator():
    ...
    def __call__(self, name: str) -> Software:
        """ Randomly chooses a software version and its associated details """
        ...
        return Software(
            name    = name,
            details = self._makeDetails(versionDetails),
            version = self._makeVersionString(majorVersion, minorVersion = minorVersion, name = name),
        )


    @staticmethod
    def _makeDetails(versionProperties : dict) -> dict:
    ...
    def _makeVersionString(majorVersion: str, **kwargs) -> str:
        """ Makes version string for a software given its name, major and minor versions. """
    ...
    @staticmethod
    def _skipMinorVersion(name: str, minorVersion: int) -> bool:
...
class UserAgentGenerator(metaclass = Singleton):
    ...
    def __call__(self, osName:str = None, brName: str = None) -> str:
