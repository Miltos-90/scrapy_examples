...
def test(ua):
    ...
    for key, value in rgx_mapper(ua, REGEXES['cpu']):
        _cpu[key] = value

    _device = {'vendor': None, 'model': None, 'type': None}
    for key, value in rgx_mapper(ua, REGEXES['device']):
        _device[key] = value

    _engine = {'name': None, 'version': None}
    for key, value in rgx_mapper(ua, REGEXES['engine']):
        _engine[key] = value

    _os = {'name': None, 'version': None}
    for key, value in rgx_mapper(ua, REGEXES['os']):
        _os[key] = value
        
    return _browser, _cpu, _device, _engine, _os
