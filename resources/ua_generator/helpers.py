...
from definitions  import BROWSER_TYPE, UNKNOWN_NAME, GENERATOR_TYPE
from typing       import Union, get_args, cast, Tuple
...
def get(by: GENERATOR_TYPE, **kwargs):
    ...
    def scrape( 
        ...
        browsers: Tuple[BROWSER_TYPE] = get_args(BROWSER_TYPE), # Names of browsers to be scraped (assumed lowercase)
    ...
    def program(limit: int = 10): # Number of agents to be generated for each template (see constants.py)
        ...
        for osName, browserName in c.TEMPLATES.keys():
            ...
            for _ in range(limit):
                ...
                for name in get_args(c.OS_TYPE)   : agent = agent.replace(f'{{{name}}}', os.version)
                for name in get_args(BROWSER_TYPE): agent = agent.replace(f'{{{name}}}', browser.version)
