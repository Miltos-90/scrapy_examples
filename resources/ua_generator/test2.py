...
import re
from parser_utils import str_mapper, trim # fix these
...
def Mapper(match: re.Match, value = None):
    ...
    elif isinstance(value, str):       return key, value
...
{   # Chrome for Android/iOS
        'regex' : r'(avast|avg)\/([\w\.]+)',
        'props' : {
            'name' : lambda match: re.sub(r'(.+)', '\\1 Secure Browser', match),
            'version'    : None
            }
    }
"""
...
# Compile all regexes
for entry in wtf:
    entry['regex'] = re.compile(entry['regex'], re.IGNORECASE)
...
if __name__ == '__main__':
    ...
    for ua in uaStrings:
        ...
        for entry in wtf:
            ...
            # try matching uastring with regexes
            matches = entry['regex'].search(ua)
                
            if matches: 
                
                for i, (key, value) in enumerate(entry['props'].items()):
                    try               : match = matches.group(i + 1)
                    except IndexError : match = None
                    
                    print('my mapper result:', key, (Mapper(match, value)))
