...
class BaseGenerator(ABC):
    ...
    def _getAttribute(self, userAgent: str, attribute: tuple) -> str:
        ...
        if attribute == ('device', ): 
            # Get device type from OS name.
            prop   = ('os', 'name')
            osName = self._getAttribute(userAgent, prop)

            return self.osTypes.get(osName)
        
        else: 
            # Get other attributes from parser
    ...
    def _add(self, userAgent: str):
        ...
        device  = self._getAttribute(userAgent, ('device', ))

        if browser in get_args(BROWSER_TYPE) and device in get_args(DEVICE_TYPE):
