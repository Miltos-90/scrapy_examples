...
from definitions  import BROWSER_TYPE, DEVICE_TYPE, GENERATOR_TYPE, MAX_USER_AGENT_SIZE
...
class Generator():
    ...
    def remove(self, userAgent: str):
        """ Removes a user agent from memory. """

        self.userAgents = {
            key : val for key, val in self.userAgents.items() if val != userAgent
            }
    ...
    def _check(successes: int, failures: int):
        ...
        if successes == 0: 
            ...
            msg    = f'Succesfully imported {successes} user agents ({round(ratio, 1)} % of total).'
