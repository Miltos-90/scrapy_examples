    ...
    def generate_user_agent(self):
        if self.platform == 'windows':
            if self.browser == 'chrome':
                ...
                template = template.replace('{chrome}', utils.version(self.browser_version))
                return template
            ...
            if self.browser == 'firefox':
                ...
                template = template.replace('{windows}', self.platform_version['major'])
                template = template.replace('{firefox}', utils.version(self.browser_version, strip_zero=True))
                return template
            ...
            if self.browser == 'firefox':
                ...
                template = template.replace('{android}', self.platform_version['major'].replace('.0', ''))
                template = template.replace('{firefox}', utils.version(self.browser_version))
                return template

        elif self.platform == 'macos':
            if self.browser == 'chrome':
                template = 'Mozilla/5.0 (Macintosh; Intel Mac OS X {macos}) AppleWebKit/{webkit} (KHTML, like Gecko) Chrome/{chrome} Safari/{webkit}'
                template = template.replace('{macos}', utils.version(self.platform_version, strip_zero=True).replace('.', '_'))
                template = template.replace('{webkit}', self.browser_version['webkit'])
                template = template.replace('{chrome}', utils.version(self.browser_version))
                return template
            if self.browser == 'edge':
            ...
            if self.browser == 'safari':
                ...
                template = template.replace('{macos}', utils.version(self.platform_version, strip_zero=True).replace('.', '_'))
                template = template.replace('{webkit}', self.browser_version['webkit'])
                template = template.replace('{safari}', utils.version(self.browser_version))
                return template
            if self.browser == 'firefox':
            ...
            if self.browser == 'chrome':
                ...
                template = template.replace('{ios}', utils.version(self.platform_version, strip_zero=True).replace('.', '_'))
                template = template.replace('{webkit}', self.browser_version['webkit'])
                template = template.replace('{chrome}', utils.version(self.browser_version))
                return template
            if self.browser == 'edge':
        ...
        raise NotImplementedError('Generator failed.')
