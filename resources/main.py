...
class HeaderGenerator(metaclass = utils.Singleton):
    ...
    def __call__(self,
        ...
        try: 
            ...
            # Blacklist this user agent -> Remove from dictionary. 
            # How to handle this for programmer? Dictionary does not exist there
            # Generate user agents during init?
            if hasattr(self.UserAgent, 'userAgents'):

                self.UserAgent.userAgents = {
                    key:val for key, val in self.UserAgent.userAgents.items() if val != userAgent
                    }
