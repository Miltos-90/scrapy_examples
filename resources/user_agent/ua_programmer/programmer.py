...
class Programmer(BaseGenerator):
    ...
    def _makeOS(self,
        ...
        isDeviceCompatible = lambda osName, deviceType: self.osTypes[osName] == deviceType

        # Get browser compatible OS
        keyCompatible  = self.browserToOS[browser]
        
        # Choose one at random if its also device compatible
        osName = choice(
            [name for name in keyCompatible if isDeviceCompatible(name, device)]
        )
