...
from ..templates       import Software, UserAgent, OS_TYPES
...
class Programmer():
    """ Generates programmatically a randomly-selected user agent according to a set type, OS, and browser """
    ...
    def _makeBrowser(self, 
        ...
        if not browser: 
            browser = choice(self.browsers) # Choose one at random if not provided

        else: # Ensure lowercase and availability
            browser = browser.lower()

            if not browser in self.browsers:
