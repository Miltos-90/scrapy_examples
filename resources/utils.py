...
def readFile(pathToFile: str) -> Any:
    ...
    if os.path.isfile(pathToFile):
        with open(pathToFile, mode = 'r', encoding = 'utf-8') as f:
            ...
            else: raise NotImplementedError(f"{pathToFile} is not a .json or .txt file")
...
class Accept(HTTPHeaderGenerator):
    ...
    def __call__(self, 
        ...
        # a list of version-from and -to numbers [ints] and header contents
        list_ = self.data[name]

        # Loop over the list and get the header value according to the version range
        for dict_ in list_:
            vFrom  = dict_["version_from"]
            vTo    = dict_["version_to"]
            header = dict_["header"]
            
            if version >= vFrom and version < vTo:
