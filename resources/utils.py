...
from typing import Union, Tuple, Any, get_args
from definitions import DEVICE_TYPE, BROWSER_TYPE
import json
import random as rd
import os
...
class Selector():
    ...
    def __init__(self):
        ...
        self.data = readFile('./data/market_share.json')

        # Make weights for device selection
        self.devices  = get_args(DEVICE_TYPE)
        self.dWeights = [self.data[d]["usage"] for d in self.devices]

        return

    def device(self) -> DEVICE_TYPE:
        """ Selects a device """
        return rd.choices(population = self.devices, weights = self.dWeights)[0]


    def browser(self, device: DEVICE_TYPE) -> BROWSER_TYPE:
        """ Selects a browser """

        # Select browser
        browser = rd.choices(
            population = list( self.data[device]["browser"].keys() ),
            weights    = list( self.data[device]["browser"].values() )
        )[0]
